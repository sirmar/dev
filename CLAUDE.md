...
- `completions/dev.bash` and `completions/_dev` are generated by `src/generate-completions.sh` — do not edit by hand.
- To add static args for a `dev` command, update `cmd_args` in `src/app/dev.sh`.
